...
fn   xor( key48:  Vec<u8>, value: Vec<u8> ) -> Vec<u8>
{
        let result   =   Vec::with_capacity(56) ;
        for   (i, j)  in  key48.iter().zip(value.iter())
                 result.push(i ^ j);      
                 
       return    result;                 
}
 
fn expansion (  input32:Vec<u8>) ->  Vec<u8> 
{
      let result   =   Vec::with_capacity(48) ;
      let tmp;
       let k=0;
      for  chunk in      input32.chunks(4)      //   8 bit  
      {
          for i in  0..4 
          {
              if(i==2)
...
fn swappper (input32: Vec<u8>) -> Vec<u8> 
    {
        let mut result = Vec::with_capacity(32);
        result.extend(&input32[16..32]);
        result.extend(&input32[0..16]);
        result
    }
